...
import time
import logging
from sqlalchemy import Table,MetaData,Column,Integer,Date,VARCHAR,Float,DateTime
...
def config_file(login_file = 'login.json'):
    ...
    try:
        with open(os.path.join(__location__,login_file)) as f:
            config = json.load(f)
            user_name,password,api = config[0]['user_name'],config[0]['password'],config[0]['api']
            return user_name,password,api
    except:
        raise
...
def links(conn,base_link):
    ...
    base_link = base_link.replace("API", api)
    ...
    if len(link_list)!=0:
        ...
        for link in link_list:
            try:
                ...
                df = df.rename(columns={"Product Code": "Market",
                                        "Settlement Date": "SettlementDate",
                                        "Settlement Price": "SettlementValue",
                                        "Instrument Start Date": "InstrumentStartDate",
                                        "Instrument End Date": "InstrumentEndDate"})
...
def main():
    ...
    prompt_month.spot_calculation(conn, engine, log)
